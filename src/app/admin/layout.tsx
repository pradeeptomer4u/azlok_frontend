...
export default function AdminLayout({
  ...
  const canAccess = (module: string) => {
    // If no permissions loaded yet, show all menu items for admin users (default behavior)
    if (!permissions) return true;
    return hasModuleAccess(permissions, module as any);
  };

  // Check if user has any admin permissions
  const hasAnyAdminAccess = permissions && (
    permissions.is_super_admin ||
    hasModuleAccess(permissions, 'blogs') ||
    hasModuleAccess(permissions, 'orders') ||
    hasModuleAccess(permissions, 'products') ||
    hasModuleAccess(permissions, 'categories') ||
    hasModuleAccess(permissions, 'users') ||
  ...
  useEffect(() => {
    if (!isLoading && !isAuthenticated) {
      router.push('/login');
    } else if (!isLoading && isAuthenticated && permissions && !hasAnyAdminAccess) {
      // User is authenticated but has no admin permissions
      router.push('/dashboard');
    }
  }, [isLoading, isAuthenticated, permissions, hasAnyAdminAccess, router]);
  ...
  if (!user || (!hasAnyAdminAccess && permissions)) {
  ...
  return (
    <div className="flex h-screen bg-gray-100 overflow-hidden">
      ...
      <div className="w-64 bg-white shadow-md flex flex-col h-full">
        ...
        <nav className="p-4 overflow-y-auto flex-1 max-h-[calc(100vh-80px)]">
          ...
          {canAccess('inventory') && (
            <>
              <p className="text-xs font-semibold text-gray-400 mt-6 mb-2">INVENTORY</p>
              <Link 
          ...
          <Link 
            href="/admin/inventory/packaged-products" 
            className="flex items-center py-2 px-3 mb-1 rounded-md hover:bg-gray-100"
          >
            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-3 text-gray-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" />
          ...
          <Link 
            href="/admin/inventory/gate-passes" 
            className="flex items-center py-2 px-3 mb-1 rounded-md hover:bg-gray-100"
          >
            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-3 text-gray-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
