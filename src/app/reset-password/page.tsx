...
export default function ResetPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('https://api.azlok.com/api/auth/reset-password', {
        method: 'POST',
        headers: {
        ...
        {!success ? (
          <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
            <div className="space-y-4">
              ...
              <div>
                <label htmlFor="confirm-password" className="block text-sm font-medium text-gray-700 mb-2 font-['Montserrat',sans-serif]">
                  Confirm Password
                </label>
                <input
                  id="confirm-password"
