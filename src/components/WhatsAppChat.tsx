    ...
    <>
      ...
      <button
        ...
        className={`flex items-center justify-center ${
          buttonStyle === 'floating'
            ? `fixed ${positionClasses[position]} z-50 shadow-lg bg-white text-black font-medium rounded-full p-3 pl-4 pr-5 hover:shadow-xl transition-all duration-300 ${isAnimating ? 'animate-pulse' : ''}`
            : 'bg-white text-black font-medium rounded-full p-3 pl-4 pr-5 hover:shadow-md transition-all duration-300'
        }`}
        aria-label="Chat on WhatsApp"
      >
        <div className="flex items-center relative">
          {/* Animated ring effect */}
          <div className={`absolute inset-0 rounded-full bg-green-500 opacity-30 scale-125 ${isAnimating ? 'animate-ping' : 'opacity-0'}`}></div>
          
          {/* WhatsApp Icon */}
          <div className="bg-gradient-to-br from-green-400 to-green-600 rounded-full p-1.5 relative">
          ...
          {/* Text with gradient */}
          <div className="ml-2 font-bold bg-clip-text text-transparent bg-gradient-to-r from-green-600 to-green-800">CHAT</div>
        </div>
      </button>
